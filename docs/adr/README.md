...
| [0084](0084-tool-resolution-and-runner-split.md) | Registry resolves (factories → per-container instances), a `ToolRunner` executes, the container is a per-call value; `run`/`execute` split, `static schema`, permission not "mode" | accepted |
