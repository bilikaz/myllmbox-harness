...
| **Stamp** | The engine-filled non-model fields riding a tool call across the bridge: `imageOutputDir`, `mediaRefs`, `sessionId`, `meta`. Tools can't reach the store — the stamp is their view of the session. The **container** rides the wire separately (it resolves the tool), not as a call field; `cwd` is gone (`this.cwd()` reads `container.config.root`). | inject · thread through |
| **Tool permission** | The `0\|1\|2` policy value on a tool: **`defaultPermission`** (the tool's own default) and **`currentPermission`** (resolved by `calculatePermission` = workspace grant clamped by agent ceiling). It is a *permission*, never a "mode". | mode · effectiveMode · defaultMode (all retired) |
| **Tool runner** | `ToolRunner` — **executes** tool calls: owns the in-flight `AbortController` map, resolves a call's tool against its container, dispatches, cancels. The **registry** only registers + resolves (`filter`/`resolve`); it does not run. | dispatcher (loose) · `registry.run` (retired) |
| **Resolve (a tool)** | Building a `BaseTool` instance for a container from its factory (`registry.filter`/`resolve`). Tools are built **fresh per call**, bound to that call's container — not instantiated once at boot. | build · instantiate (loose) |
